...
typedef struct Node {
    ...
    int weights[MAX_CHILDREN]; // Added weights for edges
} Node;
...
Node* create_node(int id, NodeType type, int heuristic) {
    ...
    for (int i = 0; i < MAX_CHILDREN; i++) {
        ...
        node->weights[i] = 0; // Initialize weights
    }
    return node;
}
...
void add_child(Node* parent, Node* child, int weight) {
    if (parent->num_children < MAX_CHILDREN) {
        ...
        parent->weights[parent->num_children] = weight; // Set weight for the edge
        parent->num_children++;
    } else {
...
int ao_star(Node* node) {
    ...
    if (node->type == OR_NODE) {
        ...
        for (int i = 0; i < node->num_children; i++) {
            int child_cost = ao_star(node->children[i]) + node->weights[i]; // Include weight
        ...
        int total_cost = 0;
        for (int i = 0; i < node->num_children; i++) {
            total_cost += ao_star(node->children[i]) + node->weights[i]; // Include weight
        }
